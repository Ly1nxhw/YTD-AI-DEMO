...
1. **初始化**：在仓库根目录运行 `.specify/scripts/bash/setup-plan.sh --json`，解析返回 JSON 中的 `FEATURE_SPEC`、`IMPL_PLAN`、`SPECS_DIR`、`BRANCH`。如果参数里包含单引号（例如 `I'm Groot`），使用正确转义。
...
3. **更新代理上下文**：
   - 在 `AGENTS.md` 中，将 `<!-- SPECKIT START -->` 与 `<!-- SPECKIT END -->` 之间的引用更新为当前 plan 文件路径
